...
def main(state):
    ...
    df_route_county = pd.read_csv(r'inputs\model\daysim\lookup\transit_route_county.csv')
...
#
    ...
    # Boardings by Pierce Transit
    df_piercetransit = df.groupby(['agency', 'route_code', 'county', 'description']).sum().reset_index()
    df_piercetransit.to_csv(
        os.path.join(validation_output_dir, "daily_boardings_by_route.csv"),
        index=False,
        columns=["agency", "model_boardings", 2023, 2024],
    )
    ...
    station_county = pd.read_csv(r'inputs\model\daysim\lookup\station_county.csv')
    ...
    df_daily = (
        ...
        .agg({"@tveh": "sum", "@facilitytype": "first", "@countyid": "first"})
        .reset_index()
    )
    ...
    df_county_facility_counts = (
        df_daily.groupby(["county", "@facilitytype"])[["observed", "modeled"]]
        .sum()
        .reset_index()
    )
    ...
    df_daily_volume = model_vol_df.groupby(['@countid', 'ij', '@countyid']).agg({'@tveh':'sum', '@facilitytype': 'first'}).reset_index()
    df_daily_volume["county"] = df_daily_volume["@countyid"].astype(int).astype('str').map(state.summary_settings.county_map)
    df_daily_volume = df_daily_volume[df_daily_volume['@countid']>0].groupby(['@countid']).agg({'@tveh':'sum', '@facilitytype': 'first', 'county':['first', 'count']}).reset_index()
    ...
    df_daily_volume = df_daily_volume[['@countid_', 'volbin', '@tveh_sum', 'vehicles', 'county_first', 'county_count']]
    df_daily_volume.columns = ['countid', 'volbin', 'modeled', 'observed', 'county', 'nlinks']
    df_daily_volume[["modeled", "observed"]] = df_daily_volume[["modeled", "observed"]].astype("int")
    ...
    df_truck_daily = model_vol_df.groupby(['@countid']).agg({'@mveh':'sum', '@hveh':'sum', '@facilitytype': 'first', '@countyid': 'first'}).reset_index()
    df_truck_daily["county"] = df_truck_daily["@countyid"].astype(int).astype('str').map(state.summary_settings.county_map)
    # Merge observed with model
    df_truck_daily = df_truck_daily.merge(truck_counts, left_on='@countid', right_on='flag')
    
    # Merge with attributes
    df_truck_daily.rename(columns={'@mveh': 'modeled_medt','@hveh': 'modeled_hvyt'}, inplace=True)
