...
def arterial_delay(emmeProject, factor):
    ...
    for node in network.nodes():
        ...
        for link in node.incoming_links():
            ...
            node.number_arts_entering_int = (
                node.number_arts_entering_int + link.arterial_flag
            )
            ...
            node.sum_arts_lane_3to5_entering_int = (
                node.sum_arts_lane_3to5_entering_int + link.lane_3to5
            )
            ...
            node.sum_arts_lanecap_3to5_entering_int = (
                node.sum_arts_lanecap_3to5_entering_int + link.lanecap_3to5
            )
            ...
            node.number_oneway_arts_entering_int = (
                node.number_oneway_arts_entering_int + link.oneway_arterial_flag
            )
    ...
    for node in network.nodes():
        ...
        for link in node.incoming_links():
            ...
            if (node.number_arts_entering_int > 2) or (
                node.number_oneway_arts_entering_int > 1
            ):
                node.cycle = 1.0 + (node.sum_arts_lane_3to5_entering_int / 8.0) * (
                    node.number_arts_entering_int / 4.0
                )
            else:
                node.cycle = 0.0
    ...
    for node in network.nodes():
        for link in node.incoming_links():
            ...
            if (
                node.sum_arts_lanecap_3to5_entering_int > 0.0
                and link.data3 != 0
                and link.data3 != 5
            ):
                link.red = (
                    ...
                    * (
                        1
                        - (node.number_arts_entering_int * link.lanecap_3to5)
                        / (2 * node.sum_arts_lanecap_3to5_entering_int)
                    )
                )
            else:
                link.red = 0.0
