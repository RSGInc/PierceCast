...
def feedback_check(emmebank_path_list, emme_settings):
    ...
    for emmebank_path in emmebank_path_list:
        ...
        dictZoneLookup = dict((value,index) for index,value in enumerate(zones))
        max_piercecounty_zone_index = dictZoneLookup[int(emme_settings.MAX_PIERCECOUNTY_TAZ)]+1
        ...
        for y in range(0, len(matrix_dict["Highway"])):
            ...
            if matrix_name not in ["tnc_inc1", "tnc_inc2", "tnc_inc3"]:
                ...
                skims_logger.info(text)
                if (change_test_pierce > emme_settings.STOP_THRESHOLD) & (change_test_rest > (emme_settings.STOP_THRESHOLD_REST)) & emme_settings.USE_PIERCECAST_STOPTHRESH_STRATEGY:
                    passed = False
                    break
                if (change_test > emme_settings.STOP_THRESHOLD) & (not emme_settings.USE_PIERCECAST_STOPTHRESH_STRATEGY):
                    passed = False
                    break
...
def calc_total_vehicles(my_project):
    ...
    my_project.network_calculator(
        "link_calculation", result="@vmt", expression='@tveh * length'
    )
    my_project.network_calculator(
        "link_calculation", result="@vht", expression='@tveh * timau / 60'
    )
